...
function callback(data, userData, res) {
  db.get_user(data.ownerId, res,
              function (userInfo, resp) {
                var ret = utils.pushArray(userInfo.friends, data.openId);
                var updateUsr = {};
                if ( ret.ret ) 
                  update_user_async(data.ownerId, {friends: ret.array});
                ...
                var ret = utils.pushArray(friends, data.ownerId);
                if ( ret.ret ) updateUsr.friends = ret.array; 
                if ( data.route == cons.ACCEPT ) {
                  var ret1 = utils.pushArray(accepts, data.actId);
                  if ( ret1.ret ) updateUsr.acceptedActvs = ret1.array;
                }
                if ( data.route == cons.FAVOR ) {
                  var ret = utils.pushArray(favors, data.actId);
                  if ( ret.ret ) updateUsr.favoredActvs = ret.array;
                }

                if ( Object.keys(updateUsr).length > 0 ) 
                  update_user_async(data.openId, updateUsr);

                if ( data.route == cons.ACCEPT && ret1.ret ) {
                  db.get_actv(data.actId, resp, 
                    function(actv, response) {
                      var ret = utils.pushArray(actv.attendeeIds, data.openId);
                      if ( ret.ret ) {
                        send_accept_notify(data.ownerId, data.actId, 
                                           data.nickName);
                        var attendee = {name: data.nickName, 
                                        avatar: data.avatar};
                        var ret1 = utils.pushArray(actv.attendees, attendee);
                        db.update_actv_async(data.actId, 
                                             {attendeeIds: ret.array,
                                              attendees: ret1.array});
                      }
                  ...
                  resp.render('accepted', { title: 'you have favored' });
             });
}
